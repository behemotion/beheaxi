...
**Repo home:** This spec is authored in `behemcp/docs/superpowers/specs/` because the `beheaxi`
...
- **`<tool> describe --json`** — the registration manifest behemcp consumes, generated from the
...
3. **Conformance: black-box subprocess runner.** `beheaxi conformance <tool-cmd>` invokes the real
   shipped binary as a subprocess and asserts the contract — the same surface behemcp uses, and
...
command → one verb. Nested command groups become space-joined names (`shelf create`), which behemcp
...
`pinned` marks the common verbs behemcp exposes as flat MCP tools; the rest are reachable via
`search_tools` / `describe_tool` / `run_tool`. `mutating` lets behemcp/agents distinguish read from
write. The output validates against `manifest_schema.json`; the conformance runner asserts that
validation, so the schema is the enforced contract behemcp builds against.
...
- Exact Typer introspection mechanics for nested groups and `Annotated[...]` parameter metadata —
  including the **verb-name collision rule** when behemcp flattens space-joined names to underscores
